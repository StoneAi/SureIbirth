    ...
    switch (dfuResponse.responseStatus) {
        ...
        case OPERATION_NOT_SUPPORTED_RESPONSE:
            if (!isPerformedOldDFU) {
                ...
                //[self performOldDFUOnLocalFile:[NSString stringWithFormat:@"%@/%@/%@",NSHomeDirectory(),@"Documents",@"s110_v0_0.29e.hex"]];
            }
            else {
...
-(void)calculateDFUTime
{
    ...
    NSLog(@"upload time in sec: %ld",self.uploadTimeInSeconds);
}
...
{
    NSLog(@"onFileOpened file size: %ld",fileSizeOfBin);
