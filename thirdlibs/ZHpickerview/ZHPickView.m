...
-(void)setUpPickView{
    ...
    pickView.backgroundColor=[UIColor colorWithRed:165.0/255 green:175.0/255 blue:176.0/255 alpha:1];
...
-(void)setUpDatePickerWithdatePickerMode:(UIDatePickerMode)datePickerMode{
   ...
   // datePicker.backgroundColor=[UIColor lightGrayColor];
        ...
        NSDate *maxdate = [[NSDate alloc]initWithTimeIntervalSinceNow:3600*24*365*10];
...
-(UIToolbar *)setToolbarStyle{
    ...
    UIBarButtonItem *lefttem=[[UIBarButtonItem alloc] initWithTitle:NSLocalizedStringFromTable(@"APPCancle",@"MyLoaclization" , @"") style:UIBarButtonItemStylePlain target:self action:@selector(remove)];
    lefttem.tintColor = [UIColor grayColor];
    UIBarButtonItem *centerSpace=[[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemFlexibleSpace target:self action:nil];
    
    UIBarButtonItem *right=[[UIBarButtonItem alloc] initWithTitle:NSLocalizedStringFromTable(@"APPSure",@"MyLoaclization" , @"") style:UIBarButtonItemStylePlain target:self action:@selector(doneClick)];
...
-(void)doneClick
{
    if (_pickerView) {
        ...
        _resultString = [selectDateFormatter stringFromDate:_datePicker.date]; // 把date类型转为设置好格式的string类型
