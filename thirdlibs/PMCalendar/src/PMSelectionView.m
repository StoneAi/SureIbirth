...
- (void)drawRect:(CGRect)rect
{
    if((_startIndex >= 0) || (_endIndex >= 0)) 
    {
        ...
        //选中的颜色
        UIColor* darkColor = [UIColor colorWithRed: 1 green: 1 blue: 1 alpha: 0.72];
        ...
        long tempStart = MAX(MIN(_startIndex, _endIndex), 0);
        long tempEnd = MAX(_startIndex, _endIndex);
        
        long rowStart = tempStart / 7;
        long rowEnd = tempEnd / 7;
        long colStart = tempStart % 7;
        int colEnd = tempEnd % 7;
        
        for (long i = rowStart; i <= rowEnd; i++)
        {
            //// selectedRect Drawing
            long thisRowStartCell = 0;
