...
//          NSLog(@"NewVersion = %@",NewVerson);
      //  NSLog(@"%@",resultDic);
        ...
        if (![latestVersion isEqualToString:appVersion]) {
            NSLog(@"需要更新呢");
//            XYAlertView *alert = [XYAlertView alertViewWithTitle:NSLocalizedStringFromTable(@"APPalret", @"MyLoaclization" , @"") message:NSLocalizedStringFromTable(@"APPNeedUpdate", @"MyLoaclization" , @"") buttons:[NSArray arrayWithObjects:NSLocalizedStringFromTable(@"APPSure", @"MyLoaclization" , @""),NSLocalizedStringFromTable(@"HistoryVC_Alret_CancleButton", @"MyLoaclization" , @""),  nil] afterDismiss:^(long buttonIndex) {
//                
//            }];
//            [alert show];
            isAppNeedUpdate = NO;
