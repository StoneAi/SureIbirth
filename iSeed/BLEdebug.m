...
-(void) realRT:(unsigned char *)buffer
{
   ...
   // NSLog(@"辐射值");
    ...
    NSLog(@"num1 = %d",num1);
    NSLog(@"辐射强度 = %d，%d,%d",num2,cfg[5],cfg[6]);
    NSLog(@"距离 = %d，%d，%d",num3,cfg[7],cfg[8]);
    NSLog(@"cfg[1]= %d",cfg[1]);
    if (cfg[1]!=7) {
        [self.delegate didread:num1 label2:num2 label3:num3 states:0];
    }
     else
         [self.delegate didread:num1 label2:num2 label3:num3 states:1];
}
